...
function ServiceCard({ service }) {
  ...
  return (
    <div className="group relative h-full flex flex-col bg-white/5 backdrop-blur-lg rounded-2xl md:p-6 p-4 border border-white/10 hover:border-blue-400/50 hover:bg-white/10 transition-all duration-300 hover:shadow-[0_8px_32px_rgba(59,130,246,0.2)] hover:-translate-y-1">
      <div className="w-12 h-12 md:w-14 md:h-14 rounded-xl bg-blue-500/20 backdrop-blur-sm flex items-center justify-center text-blue-400 mb-4 group-hover:bg-blue-500 group-hover:text-white transition-all duration-300 shrink-0">
        <IconComponent className="w-6 h-6 md:w-7 md:h-7" />
      </div>
      <h3 className="font-unbounded text-xl md:text-[22px] font-semibold text-white mb-3">
        {service.title}
      </h3>
      <p className="text-white/70 leading-relaxed text-sm md:text-base mb-6 grow">
        {service.description}
      </p>
      <Link
        ...
        className="mt-auto inline-flex items-center text-blue-400 font-medium text-sm group-hover:gap-2 transition-all duration-300"
      >
        Learn more
      ...
      <div className="container relative z-10">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
          {services.map((service, idx) => (
            <AnimateIn key={service.id} type="fade-up" delay={0.2 + (idx % 4) * 0.1} className="h-full">
